...
int main() {
    int count = 0;
    // คอมไพล์โปรแกรมหลัก
    if (system("g++ -std=c++17 main.cpp input.cpp calculate.cpp output.cpp process.cpp time.cpp -o program.exe") != 0) {
    ...
    string version = extractVersion("main.cpp"); 
    ofstream run_file("runtest.txt", ios::app);
    run_file << "testresult" << endl << "version : " << version << endl << "Total cases: " << (test_case-1) << endl << "pass: " << count << " / " << (test_case-1) << endl << (all_passed ? "All passed!" : "Some failed!") << endl << "--------------------" << endl; 
    // ลบไฟล์ชั่วคราว
